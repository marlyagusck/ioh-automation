  ...
  if (result.rows) {
    result.rows.forEach(function(r) {
      rows.push({
        ...
        bytes: Number(r.f[10].v || 0),
        slot_ms: Number(r.f[11].v || 0),
        cc_email: r.f[12] ? r.f[12].v : ''
      });
    });
  }
...
function createAutoSendTrigger() {
  // Only enable this when you want scheduled auto-send.
  // ScriptApp.newTrigger('runAutoSendBatch')
  //   .timeBased()
  //   .everyHours(1)
  //   .create();
}
